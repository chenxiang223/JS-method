...
class SpatialFrequencyFusionCore(nn.Module):
    ...
    def forward(
        ...
        if not self.use_spatial_branch and not self.use_frequency_branch:
            if return_states:
                raise ValueError("Independent evidence requires both spatial and frequency branches.")
            return x
        ...
        if return_states:
            if not (self.use_spatial_branch and self.use_frequency_branch):
                raise ValueError("Independent evidence requires both spatial and frequency branches.")
            fused = (self.sfid(spa_feat, fre_feat, z_spa=z_spa, z_spe=z_spe, token_hw=token_hw)
                     if self.use_tsfi and self.sfid is not None else 0.5 * (spa_feat + fre_feat))
            return fused, {"spatial": spa_feat, "frequency": fre_feat}
...
class SPARCNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, targets: Optional[torch.Tensor] = None,
        ...
        if return_states and self.backbone_mode != "innovation1":
            raise ValueError("Six-domain states require the SPARC innovation1 backbone.")
        if self.backbone_mode == "baseline":
            ...
            z_spa = adapter_out.z_spa
        if return_states:
            return {"raw": raw_hint, "early": early_feature,
                    **branch_states, "fused": fused, "final": final_feat}
        head_output = None
    ...
    def forward_states(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        """Six evidence maps; fused has exactly the existing mid_feature semantics.

        This path bypasses the classifier and adds no parameters or checkpoint keys.
        """
        return self.forward(x, return_states=True)
